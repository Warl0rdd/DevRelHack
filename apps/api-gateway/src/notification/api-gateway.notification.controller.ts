...
export class NotificationController {
  ...
  async mailSingle(@Body() mailSingleDto: MailSingleDto) {
    const result = (await this.apiGatewayService.mailSingle(
      mailSingleDto,
    )) as any;
    if (!result.success)
      throw new HttpException(result.error.message, result.error.statusCode);
  ...
  async mailMultiple(@Body() mailMultipleDto: MailMultipleDto) {
    const result = (await this.apiGatewayService.mailMultiple(
      mailMultipleDto,
    )) as any;
    if (!result.success)
      throw new HttpException(result.error.message, result.error.statusCode);
  ...
  async telegramSingle(@Body() telegramSingleDto: TelegramSingleDto) {
    const result = (await this.apiGatewayService.telegramSingle(
      telegramSingleDto,
    )) as any;
    if (!result.success)
      throw new HttpException(result.error.message, result.error.statusCode);
  ...
  async telegramMultiple(@Body() telegramMultipleDto: TelegramMultipleDto) {
    const result = (await this.apiGatewayService.telegramMultiple(
      telegramMultipleDto,
    )) as any;
    if (!result.success)
      throw new HttpException(result.error.message, result.error.statusCode);
